  ...
  return (
    <CounterWrapper>
      <TopDiv aria-label={`Remove counter number ${index + 1}`}>
        <CounterCount $buttonClicks={clicks}>
          Counter number {index + 1}
        </CounterCount>
        <ButtonRemove onClick={removeCounter} disabled={counters <= 1}>
          Remove counter
        </ButtonRemove>
      </TopDiv>
      <ClickCount
